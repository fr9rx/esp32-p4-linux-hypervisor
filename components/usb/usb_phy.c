...
/* Not defined for the ESP32-P4 in ESP-IDF v6.1-beta1's soc_caps.h -- the cap
 * came in with the light-sleep USB isolation workaround and is absent here. An
 * undefined identifier evaluates to 0 in #if, so the guarded blocks would
 * compile out anyway; saying so explicitly keeps that from looking accidental,
 * and it is the right value: nothing in this project uses light sleep. */
#ifndef SOC_USB_UTMI_PHY_NO_POWER_OFF_ISO
#define SOC_USB_UTMI_PHY_NO_POWER_OFF_ISO 0
#endif
