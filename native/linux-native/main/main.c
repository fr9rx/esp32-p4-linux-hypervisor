...
/*
 * Leave the MIPI DSI clock tree running for Linux.
 *
 * The kernel has no ESP32-P4 clock-controller driver: its DSI nodes use
 * fixed-clock stubs and the DT comments say the boot shim leaves the DSI
 * clocks on. That was true by accident -- it relied on whatever ESP-IDF's
 * startup happened to leave enabled, and nothing here ever programmed them.
 *
 * On this board that assumption does not hold. The D-PHY PLL never locked
 * (PHY_STATUS=0x1528, bit0 clear), so every DCS write timed out with -110
 * and the panel was never initialised. The PLL reference clock
 * (reg_mipi_dsi_dphy_pll_refclk_en) is the one that matters: with it gated
 * the PLL has no input and can never lock, no matter how M/N are programmed.
 *
 * The kernel's PHY driver programs M=50 N=1 against an assumed 20 MHz
 * reference, so PLL_F20M and divider 1 are what it expects to find here.
 */
static void boot_shim_init_dsi_clocks(void)
{
    /* Every one of these LL helpers touches shared CLKRST registers, so IDF
     * requires the whole sequence to run in the RCC atomic environment. */
    PERIPH_RCC_ATOMIC() {
        _mipi_dsi_ll_enable_bus_clock(0, true);
        mipi_dsi_ll_reset_register(0);

        /* PHY config clock: PLL_F20M (MIPI_DSI_PHY_CFG_CLK_SRC_DEFAULT). */
        _mipi_dsi_ll_set_phy_config_clock_source(0, MIPI_DSI_PHY_CFG_CLK_SRC_PLL_F20M);
        mipi_dsi_ll_enable_phy_config_clock(0, true);

        /* PLL reference: PLL_F20M undivided = 20 MHz, which is what the
         * kernel PHY driver's M=50 / N=1 assumes. */
        _mipi_dsi_ll_set_phy_pllref_clock_source(0, MIPI_DSI_PHY_PLLREF_CLK_SRC_PLL_F20M);
        mipi_dsi_ll_set_phy_pll_ref_clock_div(0, 1);
        mipi_dsi_ll_enable_phy_pllref_clock(0, true);
    }

    ESP_LOGI(TAG, "MIPI DSI clocks on: bus + DPHY cfg + PLL ref (PLL_F20M/1)");
}
void app_main(void)
{
    boot_shim_kick_c6();
    boot_shim_init_sdmmc();
    boot_shim_init_dsi_clocks();
